...
def parse_inputs():
    ...
    if state == None:
	    state = raw_input("Give a status please: ")
    if host == None:
        host = raw_input("Give the EM Hostname please: ")
    username = args.username
    if username == None:
        username = raw_input("Give your EM username please: ")
...
def list_jobs(token, baseurl):
    ...
    if state=="All":
       while True:
           ...
           selected = raw_input("Enter a number to see it's output or 'q' for quit: ")
    ...
    selected2 = raw_input("Type 'r' for run now 'rr' for rerun 'h' for hold or 'q' for quit: ")
	
    if selected2 == 'q':# MP
       logout(token, baseurl)
    
    if selected2 == 'r':# MP
       selected = raw_input("Type a number: ")
       string = ''.join(r1)
       match = re.findall(r"'[A-Z][A-Z][A-Z][A-Z][A-Z][0-9][A-Z]\D.{5}'", string)
       y = 0
       y = int(selected) - 1
       jobid = match[y].strip("''")
       runnow(jobid)
...
def rerun(jobid):# MP
    ...
    r4 = requests.post(baseurl + 'run/job/' + jobid + '/rerun', headers={'Authorization': 'Bearer ' + token}, verify=False)
    
    print(r4.text)

def runnow(jobid):                                                                                                                                                                      
    
    r5 = requests.post(baseurl + 'run/job/' + jobid + '/runNow', headers={'Authorization': 'Bearer ' + token}, verify=False)
    
    print(r5.text)	
	
def hold(jobid):                                                                                                                                                                      
    
    r6 = requests.post(baseurl + 'run/job/' + jobid + '/hold', headers={'Authorization': 'Bearer ' + token}, verify=False)
    
    print(r6.text) # For more actions https://docs.bmc.com/docs/automation-api/9181/services-784100995.html
