...
def list_jobs(token, baseurl):
    ...
    values = ','.join(str(v) for v in statuses) # MP
    ...
    """if state=="All":
       while True:
           ...
           print_output(outputurl)""" # All you can eat !
    ...
    if state=="Status Unknown":
       ...
       for line in r1:
           ...
           id = Type[3]
           x += 1
           print(str(x) + '. ' + (status) + ', ' + (name) + ', ' + (id))""" # All the crazy **** I did tonight Those will be the best memories
    ...
    if action == 'hold':
       ...
       y = int(selected) - 1
       jobid = match[y].strip("''")
       hold(jobid)""" # All the crazy **** I did tonight Those will be the best memories
       x = 0
       for line in r1:
           Type = line.split("'")
           jobid = Type[29]
